...
- Task display is limited to `Production Time`, `PreProduction`, and regular `Task` task types. Workspace tasks use the existing server-side type filters with a client-side fallback.
- Tasks whose home location is private but which are added to an artist List through Tasks in Multiple Lists are not reliably returned by the Workspace task endpoint. The app also enumerates Lists in the `Production TimeTracking` Folder (`901210025033`) and calls `GET /list/{list_id}/task?include_timl=true` for each List. Results are filtered by assignee and task type, merged by task ID, and displayed under the artist-facing List.
...
- The history fetch ends at the next local midnight rather than the current instant, so entries starting later today are included. After saving, the app also keeps the POST response visible if ClickUp's GET endpoint has not propagated it yet.
- Tasks, list URLs, and full time-entry history are cached in memory for the current page session to reduce ClickUp rate-limit errors. Time-entry cache is invalidated after edits; reload the page to refresh task/list data.
